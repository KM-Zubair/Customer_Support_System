...
def chat():
    ...
    return render_template('index.html', response=response, products=products, languages=languages)
...
def generate_comment():
    ...
    if not product_info:
        return render_template('index.html', response="Product not found", products=products, languages=languages)

    # Generate customer comment
    comment = generate_customer_comment(product_info, selected_language)
    return render_template('index.html', comment=comment, products=products, languages=languages, selected_language=selected_language)
...
def generate_subject():
    products = load_products()
    # Safely retrieve the selected language
    selected_language = request.form.get('language', 'English')  # Default to English if missing
    customer_comment = request.form.get('comment')
    
    # Check if the comment is provided
    if not customer_comment:
        return render_template('index.html', comment=customer_comment, subject="No comment provided.", products=products, languages=languages, selected_language=selected_language)
    
    # Generate the email subject based on the customer comment
    subject = generate_email_subject(customer_comment, selected_language)
    return render_template('index.html', comment=customer_comment, subject=subject, products=products, languages=languages, selected_language=selected_language)
...
def generate_summary():
    ...
    # Generate the summary based on the customer comment
    summary = generate_comment_summary(customer_comment, selected_language)
    return render_template('index.html', comment=customer_comment, summary=summary, products=products, languages=languages, selected_language=selected_language)
...
def analyze_sentiment():
    ...
    return render_template('index.html', comment=customer_comment, sentiment=sentiment, products=products, languages=languages, selected_language=selected_language)
...
def generate_email():
    ...
    # Generate the full email based on the inputs
    email = generate_customer_email(customer_comment, summary, sentiment, email_subject, selected_language)
    return render_template('index.html', comment=customer_comment, summary=summary, sentiment=sentiment, subject=email_subject, email=email, products=products, languages=languages, selected_language=selected_language)
